...
class Randium_3d_gpu:
    def __init__(
        ...
        self.blocks = blocks
        self.tiles = tiles
        self.Lx = tiles[0] * blocks[0] * threads_per_block[0]
        self.Ly = tiles[1] * blocks[1] * threads_per_block[1]
        self.Lz = tiles[2] * blocks[2] * threads_per_block[2]
        ...
        self.coordination_num = backend.coordination_num
        self.lattice_type = backend.lattice_type
        ...
        self.lattice = self.lattice.reshape(
            (self.Lx, self.Ly, self.Lz)
        )
        self.d_lattice = numba.cuda.to_device(self.lattice)

        self.displacements = np.zeros(
            3 * self.Lx * self.Ly * self.Lz, 
            dtype=np.int64
        )
        ...
        tile_size = self.tiles[0] * self.tiles[1] * self.tiles[2]
    ...
    def __repr__(self):
        out = ('Randium_3d_gpu(threads_per_block='
                f'({self.threads_per_block[0]}, '
                f'{self.threads_per_block[1]}, '
                f'{self.threads_per_block[2]}), ')
        out += f'blocks=({self.blocks[0]}, {self.blocks[1]}, {self.blocks[2]}), '
        out += f'tiles=({self.tiles[0]}, {self.tiles[1]}, {self.tiles[2]}), '
    ...
    def __str__(self):
        out = self.__repr__() + '\n'
        out += (f'  System size: {int(self.Lx)} x '
                f'{int(self.Ly)} x {int(self.Lz)} = {int(self.N)}\n')
    ...
    def get_particle_lattice_displacements(self):
        ...
        dx = self.displacements[::3].reshape((self.Lx, self.Ly, self.Lz))
        dy = self.displacements[1::3].reshape((self.Lx, self.Ly, self.Lz))
        dz = self.displacements[2::3].reshape((self.Lx, self.Ly, self.Lz))
    ...
    def update_data_log(self):
        ...
        dx, dy, dz, dr, x, y, z = self.get_particle_lattice_displacements()
    ...
    def benchmark(self):
        ...
        mc_attempts_per_step = self.Lx * self.Ly * self.Lz
    ...
    def meta_info_simple(self):
        ...
        return dict(
            ...
            N_M     = int(self.N_M),
            z       = int(self.coordination_num),
            lattice = str(self.lattice_type)
        )
